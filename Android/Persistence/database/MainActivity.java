查看数据库
    .把在 sdk 目录下的 platform-tools 路径添加到 系统变量 path
    .打开 cmd 输入 adb shell
    ...
    .输入 cd /data/data/com.example.database/databases/      com.example.database 是包名
        如果提示 No such file or directory 可能是因为没创建数据库
    .输入 ls 查看清单，
    .输入 sqlite3 BookStore.db 打开数据库 
            ...
            如果 sqlite 出现 ...> 输入 ; 并按回车
    .输入 exit  退出设备控制台
    .adb devices 显示设备有哪些
    
创建一个自己的帮助类去继承它
    SQLiteOpenHelper 是一个抽象类
    SQLiteOpenHelper 中有两个抽象方法
            onCreate() --创建
            onUpgrade() -- 升级数据库
...
构建出实例
    ...
    这个构造方法中接收4个参数，
        参数1 是Context，
        参数2 是数据库名，创建数据库时使用的就是这里指定的名称。
        参数3 允许我们在查询数据的时候返回一个自定义的 Cursor ，一般都是传入null 。
        参数4 表示当前数据库的版本号，可用于对数据库进行升级操作。

创建数据库
    调用 getReadableDatabase() 或 getWritableDatabase() 方法创建数据库，重写的 onCreate() 方法也会得到执行，
...
MyDatabaseHelper.java
    ...
    使用 primary key 将id 列设为主键，并用 autoincrement 关键字表示id 列是自增长的。

    调用了 SQLiteDatabase 的 execSQL() 方法去建表
    没有 BookStore.db 这个数据库，会创建该数据库并调用 MyDatabaseHelper 中的 onCreate()，有就不创建不调用
...
数据进行的操作4种，即CRUD。
    ...
    insert() 用于添加数据的，每次点击都会创建新的
        ...
        参数3 是一个 ContentValues 对象，它提供了一系列的put() 方法重载，用于向ContentValues 中添加数据，
            ...
            因为将id 列设置为自增长了，它的值会在入库的时候自动生成
    update() 用于对数据进行更新
        参数1 表名，
        参数2 ContentValues 对象，要把更新数据在这里组装进去。
        参数3、参数4 用于约束更新某一行或某几行中的数据，不指定的话默认就是更新所有行。
    ...
    delete() 专门用于删除数据，
        参数1 表名，
        参数2、参数3 用于约束删除某一行或某几行的数据，不指定的话默认就是删除所有行。
        范例通过第二、第三个参数来指定仅删除那些页数超过500页的书
    query() 方法用于对数据进行查询，最短的一个方法重载也需要传入7个参数。
        参数1 表名
        参数2 用于指定去查询哪几列，如果不指定则默认查询所有列。
        参数3、参数4 用于约束查询某一行或某几行的数据，不指定则默认查询所有行的数据。
        参数5 用于指定需要去group by的列，不指定则表示不对查询结果进行group by操作。
        参数6 用于对group by之后的数据进行进一步的过滤，不指定则表示不进行过滤。
        参数7 用于指定查询结果的排序方式，不指定则表示使用默认的排序方式
